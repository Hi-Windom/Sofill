...
async function 以sql向思源请求块数据(sql) {
    ...
    return util.parseResponse(util.post2Siyuan(url, sqldata));
}
...
async function 以id获取思源块属性(内容块id) {
    ...
    return util.parseResponse(util.post2Siyuan(url, data));
}
...
async function 设置思源块属性(内容块id, 属性对象) {
    ...
    return util.parseResponse(util.post2Siyuan(url, {
        id: 内容块id,
        attrs: 属性对象,
    }));
}
...
async function 插入块(previousID, dataType, data) {
    ...
    return util.parseResponse(util.post2Siyuan((url = url), (data = {
